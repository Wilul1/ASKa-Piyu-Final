...
def _build_user_prompt(
    *,
    ...
    return (
        ...
        "- When context contains a structured table, each record's Identifier, Range, and Status belong together. A Status/interpretation label is authoritative for that Identifier only.\n"
        "- Do not assume a larger identifier or larger number means 'above', 'better', or 'passing' unless a label on that same record says so. Some institutional scales use smaller codes for better outcomes.\n"
        "- Do not write 'X and above' or treat a Failed/Conditional/Incomplete status as a passing or successful outcome. If the table does not use the word passing, do not invent a passing threshold.\n"
        "- Do not quote extraction placeholders such as [NEEDS REVIEW], form. N, or orphan dash-numbers as fees or facts. If a value is not clearly specified, say so.\n"
