...
if not exist "android\key.properties" (
  ...
  echo WARNING: Signing release APK with DEBUG keystore ^(local only^).
)

echo Building Flutter APK ^(ph.edu.lspu.aska_piyu^) with ASKA_API_BASE_URL=%ASKA_API_BASE_URL%
flutter build apk --release --dart-define=ASKA_API_BASE_URL=%ASKA_API_BASE_URL%
exit /b %ERRORLEVEL%
